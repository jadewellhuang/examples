...
這個 repo 是一堆互相獨立的小範例，日常任務多半很小。**三行的修改不需要四個 subagent。**規則的地板是：誠實回報三態、改過的 .py 跑 py_compile、做完 push——這三件事再小的任務都要做。至於派工、verifier、模板，觸發條件寫在 delegation.md 第 1 節，**沒觸發就不要演**。制度是拿來省 token 和防錯的；如果你發現自己為了走流程花的 token 比做事還多，那是在錯用制度（也請把該案例記進 lessons.md）。
...
- 假設本 repo 之後的工作型態仍以「小範例的修改與問答」為主；若開始出現大型多檔開發，delegation.md 的閾值（400 行／3 檔）可能要調，先問使用者。
