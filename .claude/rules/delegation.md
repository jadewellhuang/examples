...
- 要讀的內容預估超過 200 行，或要碰超過 3 個檔案（與 CLAUDE.md 第 1 條的 200 行上限同一個數字）
...
- 回報上限預設 30 行；例外：研究型 60 行、審查與第二意見 40 行，要在派工時明說。
...
**微改動例外（比例原則）**：同時滿足「單一檔案、diff ≤10 行、驗證階梯第 1–2 層已親自跑過」的改動，可免派 verifier，但回報必須寫明「微改動，依比例原則未派驗收」。不同時滿足三條件，就照下面的規則派。
